...
分布式编程是一门能让你在多台计算机上解决问题就如同在一台主机上一样的艺术 —— 通常情况下是因为单台计算机已经不足以解决当前问题了。
...
在理想情况下，添加一台新机器将使系统的性能和容量呈线性增加。但这显然是不可能的，因为计算机会产生有一些额外的开销。数据需要被复制，计算任务需要被协调，诸如此类。这就是为什么研究分布式算法是值得的——它们为具体问题提供了有效的解决方案，除此之外也指导了什么是可行的，正确实现的最小成本是什么以及什么是不可行的。
...
除了以上这些趋势之外——这是由物理约束导致的结果——是系统设计方案的世界。
