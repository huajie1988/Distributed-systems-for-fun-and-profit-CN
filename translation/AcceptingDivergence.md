...
我们不采用字符串拼接的方式，而是假设我们正在寻找一组整数的最大值（如MAX()）。假设消息1、2、3是：
...
这意味着一些熟悉的数据类型具有更特殊的实现，例如CRDT的实现，它进行了不同的折衷，以便以顺序无关的方式解决冲突。不同于用简单处理记录的键值存储（例如从系统的角度看值是不透明的二进制对象），使用CRDT的人必须使用正确的数据类型来避免异常。
